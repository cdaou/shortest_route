...
public class drawGraph extends JComponent {
	...
	public void paintComponent(Graphics g){
		...
		for (Map.Entry<String, Node> v : nodeList.entrySet()) {
			...
			g2.drawString(v.getValue().getName(), nodeX+radius/2, nodeY+2*radius-5);
		}
	}
	...
	public void drawArrows(Graphics2D g, int x1, int y1, int x2, int y2, Color arrowColor, int thickness) {
		...
		final int height = radius/2 + thickness;	
		final int dx = x2 - x1;
		final int dy = y2 - y1;
		final double square_root = Math.sqrt(dx*dx + dy*dy);
		double xm = square_root - d, xn = xm, ym = height, yn = -height, x;
	...
	public void setShortestPath(ArrayList<Node> path){
		this.path = path;
	}
