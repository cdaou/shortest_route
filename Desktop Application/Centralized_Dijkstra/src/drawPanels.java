...
public class drawPanels extends JFrame{
	...
	private DijkstraShortestPath shortestPath = new DijkstraShortestPath();
	...
	public drawPanels() {
		...
		middlePanel.add(mapViewer,  new Integer(0));
		...
		middlePanel.add(back,  new Integer(1));
		middlePanel.repaint();
		//END
					...
					if(e.getButton() == MouseEvent.BUTTON3){
						...
						delete.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent deleteEvent) { 
								for (Map.Entry<String, Node> n : nodeList.entrySet()) {
									tempList = new ArrayList<Edge>(n.getValue().getAdjacenciesList());
									for(Edge ed : n.getValue().getAdjacenciesList()){
										if(ed.getDestinationNode().getName().equals(v.getValue().getName()) || ed.getSourceNode().getName().equals(v.getValue().getName())){
											tempList.remove(ed);
									...
									n.getValue().setAdjacenciesList(tempList);
								}
						...
						rename.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent renameEvent) {
								...
								if(newNodeName!=null && newNodeName.length()>0) {
									if(!nodeList.containsKey(newNodeName)) {
										...
										JOptionPane.showMessageDialog(null,"This name already exist. Try another!");
									}
								}	
							}
						}); 
						...
						if(destinationList.contains(v.getKey())) {
							if(!sourceInput.isEmpty()) {
								...
								dG.setPathIndex(pathIndex);
								dG.repaint();
								if (nodeList.get(destinationList.get(pathIndex)).getDistance() < Double.MAX_VALUE){
									labelPathInfo.setText(shortestPath.getShortestPathTo(nodeList.get(destinationList.get(pathIndex)))+"   \u2022  "+String.format( "%.2f", nodeList.get(destinationList.get(pathIndex)).getDistance(), 2) );
								}else{
									labelPathInfo.setText("There is no path connecting: "+sourceInput+" to "+v.getKey());
								}
								pathsPanel.repaint();
								middlePanel.revalidate();
								middlePanel.repaint();
			...
			public void actionPerformed(ActionEvent e) { 
				if(nodeList.containsKey(givenSource.getText())){
					...
					if(!destinationList.isEmpty()){
						...
						shortestPath.computeShortestPaths(nodeList.get(sourceInput));
						dG.clearPaths();
						if (nodeList.get(destinationList.get(pathIndex)).getDistance() < Double.MAX_VALUE){
							labelPathInfo.setText(shortestPath.getShortestPathTo(nodeList.get(destinationList.get(pathIndex)))+"   \u2022  "+String.format( "%.2f", nodeList.get(destinationList.get(pathIndex)).getDistance(), 2) );
						}else{
						...
						for(String destination : destinationList) {
							if (nodeList.get(destination).getDistance() < Double.MAX_VALUE){
								dG.addShortestPath(shortestPath.getShortestPathTo(nodeList.get(destination)));
							}else{
								dG.addShortestPath(new ArrayList<Node>());
							}
						}
						dG.setPathIndex(pathIndex);
						dG.revalidate();
